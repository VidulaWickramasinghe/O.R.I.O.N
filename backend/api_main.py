...
try:
    ...
    def research_public_page(url: str) -> Dict[str, Any]:
        return {"status": "unavailable", "url": url, "summary": "Browser research dependencies are unavailable."}

    def compare_web_pages(urls: List[str]) -> str:
        return "Browser comparison is unavailable because browser research dependencies are missing."
...
def system_doctor():
    ...
    report = render_system_doctor_report(result)
    log_activity(
        "SYSTEM_DOCTOR",
        f"System doctor completed. Status: {result['status']}. Passed: {result['passed']}. Failed: {result['failed']}.",
        "O.R.I.O.N.",
    )
