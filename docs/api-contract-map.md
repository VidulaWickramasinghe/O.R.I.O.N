...
| Desktop | `POST /api/desktop/workspaces/{id}/{open-vscode,open-folder,start-dev}`, `POST /api/desktop/open-url`, `GET /api/sidecar/status`, `GET /api/desktop-shell/status` | Side effects use the Capability Gateway. Backend lifecycle is owned only by the Tauri Rust supervisor. |
| Persistence | `GET /api/system/persistence`, `POST /api/system/persistence/backups`, `POST /api/system/persistence/backups/{backup_id}/restore-request` | Backups are hash-verified SQLite snapshots. Restore creates a critical approval and applies only during the next backend startup. |
